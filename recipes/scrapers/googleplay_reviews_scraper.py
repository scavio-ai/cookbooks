"""
Google Play app reviews scraper -- pure Scavio API scraper.

Scrapes data straight from the Scavio API endpoint POST /api/v1/googleplay/reviews.
...
Usage:
  python googleplay_reviews_scraper.py "com.google.android.apps.maps"
"""
...
API_URL = "https://api.scavio.dev/api/v1/googleplay/reviews"


def scrape(app_id: str) -> dict:
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python googleplay_reviews_scraper.py 'com.google.android.apps.maps'", file=sys.stderr)
        raise SystemExit(1)
    app_id = sys.argv[1]
    data = scrape(app_id)
