"""
Yelp business detail scraper -- pure Scavio API scraper.

Scrapes data straight from the Scavio API endpoint POST /api/v1/yelp/business.
...
Usage:
  python yelp_business_scraper.py "starbucks-austin"
"""
...
API_URL = "https://api.scavio.dev/api/v1/yelp/business"


def scrape(business_id: str) -> dict:
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python yelp_business_scraper.py 'starbucks-austin'", file=sys.stderr)
        raise SystemExit(1)
    business_id = sys.argv[1]
    data = scrape(business_id)
