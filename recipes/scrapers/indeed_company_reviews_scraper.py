"""
Indeed company reviews scraper -- pure Scavio API scraper.

Scrapes data straight from the Scavio API endpoint POST /api/v1/indeed/company/reviews.
...
Usage:
  python indeed_company_reviews_scraper.py "Google"
"""
...
API_URL = "https://api.scavio.dev/api/v1/indeed/company/reviews"
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python indeed_company_reviews_scraper.py 'Google'", file=sys.stderr)
        raise SystemExit(1)
    company = sys.argv[1]
    data = scrape(company)
