"""
TikTok Shop category products scraper -- pure Scavio API scraper.

Scrapes data straight from the Scavio API endpoint POST /api/v1/tiktok-shop/category/products.
...
Usage:
  python tiktok_shop_category_products_scraper.py "601101"
"""
...
API_URL = "https://api.scavio.dev/api/v1/tiktok-shop/category/products"


def scrape(category_id: str) -> dict:
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python tiktok_shop_category_products_scraper.py '601101'", file=sys.stderr)
        raise SystemExit(1)
    category_id = sys.argv[1]
    data = scrape(category_id)
