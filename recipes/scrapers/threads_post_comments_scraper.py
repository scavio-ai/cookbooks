"""
Threads post comments scraper -- pure Scavio API scraper.

Scrapes data straight from the Scavio API endpoint POST /api/v1/threads/post/comments.
...
Usage:
  python threads_post_comments_scraper.py "C_abc123"
"""
...
API_URL = "https://api.scavio.dev/api/v1/threads/post/comments"
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python threads_post_comments_scraper.py 'C_abc123'", file=sys.stderr)
        raise SystemExit(1)
    post_id = sys.argv[1]
    data = scrape(post_id)
