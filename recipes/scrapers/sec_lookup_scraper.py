"""
SEC EDGAR company lookup scraper -- pure Scavio API scraper.

Scrapes data straight from the Scavio API endpoint POST /api/v1/sec/lookup.
...
Usage:
  python sec_lookup_scraper.py "apple"
"""
...
API_URL = "https://api.scavio.dev/api/v1/sec/lookup"
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python sec_lookup_scraper.py 'apple'", file=sys.stderr)
        raise SystemExit(1)
    query = sys.argv[1]
    data = scrape(query)
