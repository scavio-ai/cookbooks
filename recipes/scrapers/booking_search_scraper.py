"""
Booking.com search scraper -- pure Scavio API scraper.

Scrapes data straight from the Scavio API endpoint POST /api/v1/booking/search.
...
Usage:
  python booking_search_scraper.py "Austin, Texas"
"""
...
API_URL = "https://api.scavio.dev/api/v1/booking/search"


def scrape(destination: str) -> dict:
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python booking_search_scraper.py 'Austin, Texas'", file=sys.stderr)
        raise SystemExit(1)
    destination = sys.argv[1]
    data = scrape(destination)
