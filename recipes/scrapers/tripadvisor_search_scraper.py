"""
TripAdvisor search scraper -- pure Scavio API scraper.

Scrapes data straight from the Scavio API endpoint POST /api/v1/tripadvisor/search.
...
Usage:
  python tripadvisor_search_scraper.py "best hotels Austin"
"""
...
API_URL = "https://api.scavio.dev/api/v1/tripadvisor/search"
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python tripadvisor_search_scraper.py 'best hotels Austin'", file=sys.stderr)
        raise SystemExit(1)
    query = sys.argv[1]
    data = scrape(query)
