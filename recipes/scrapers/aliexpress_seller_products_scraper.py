"""
AliExpress seller products scraper -- pure Scavio API scraper.

Scrapes data straight from the Scavio API endpoint POST /api/v1/aliexpress/seller-products.
...
Usage:
  python aliexpress_seller_products_scraper.py "231380092"
"""
...
API_URL = "https://api.scavio.dev/api/v1/aliexpress/seller-products"


def scrape(store_id: str) -> dict:
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python aliexpress_seller_products_scraper.py '231380092'", file=sys.stderr)
        raise SystemExit(1)
    store_id = sys.argv[1]
    data = scrape(store_id)
