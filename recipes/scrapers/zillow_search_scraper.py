"""
Zillow property search scraper -- pure Scavio API scraper.

Scrapes data straight from the Scavio API endpoint POST /api/v1/zillow/search.
...
Usage:
  python zillow_search_scraper.py "austin-tx"
"""
...
API_URL = "https://api.scavio.dev/api/v1/zillow/search"


def scrape(location: str) -> dict:
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python zillow_search_scraper.py 'austin-tx'", file=sys.stderr)
        raise SystemExit(1)
    location = sys.argv[1]
