"""
Twitch channel profile scraper -- pure Scavio API scraper.

Scrapes data straight from the Scavio API endpoint POST /api/v1/twitch/profile.
...
Usage:
  python twitch_profile_scraper.py "ninja"
"""
...
API_URL = "https://api.scavio.dev/api/v1/twitch/profile"
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python twitch_profile_scraper.py 'ninja'", file=sys.stderr)
        raise SystemExit(1)
    handle = sys.argv[1]
    data = scrape(handle)
