"""
LinkedIn ad detail scraper -- pure Scavio API scraper.

Scrapes data straight from the Scavio API endpoint POST /api/v1/linkedin/ads/detail.
...
Usage:
  python linkedin_ads_detail_scraper.py "1564460026"
"""
...
API_URL = "https://api.scavio.dev/api/v1/linkedin/ads/detail"


def scrape(ad_id: str) -> dict:
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python linkedin_ads_detail_scraper.py '1564460026'", file=sys.stderr)
        raise SystemExit(1)
    ad_id = sys.argv[1]
    data = scrape(ad_id)
