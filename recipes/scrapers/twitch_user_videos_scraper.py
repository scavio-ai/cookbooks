"""
Twitch user videos scraper -- pure Scavio API scraper.

Scrapes data straight from the Scavio API endpoint POST /api/v1/twitch/user/videos.
...
Usage:
  python twitch_user_videos_scraper.py "ninja"
"""
...
API_URL = "https://api.scavio.dev/api/v1/twitch/user/videos"
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python twitch_user_videos_scraper.py 'ninja'", file=sys.stderr)
        raise SystemExit(1)
    handle = sys.argv[1]
    data = scrape(handle)
