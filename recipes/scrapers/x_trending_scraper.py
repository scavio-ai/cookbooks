"""
X (Twitter) trending topics scraper -- pure Scavio API scraper.

Scrapes data straight from the Scavio API endpoint POST /api/v1/x/trending.
...
Usage:
  python x_trending_scraper.py "UnitedStates"
"""
...
API_URL = "https://api.scavio.dev/api/v1/x/trending"


def scrape(country: str) -> dict:
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python x_trending_scraper.py 'UnitedStates'", file=sys.stderr)
        raise SystemExit(1)
    country = sys.argv[1]
    data = scrape(country)
