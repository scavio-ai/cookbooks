"""
TikTok user search scraper -- pure Scavio API scraper.

Scrapes TikTok creators matching a keyword,
straight from the Scavio API endpoint POST /api/v1/tiktok/search/users. No SDK, no
...
Usage:
  python tiktok_user_search_scraper.py "coffee"
"""
...
API_URL = "https://api.scavio.dev/api/v1/tiktok/search/users"
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python tiktok_user_search_scraper.py <keyword>", file=sys.stderr)
        raise SystemExit(1)
    keyword = sys.argv[1]
    data = scrape(keyword)
