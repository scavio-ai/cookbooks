"""
Amazon search scraper -- pure Scavio API scraper.

Scrapes Amazon product search results with price, rating, and reviews,
straight from the Scavio API endpoint POST /api/v1/amazon/search. No SDK, no
...
Usage:
  python amazon_search_scraper.py "yoga mat"
"""
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python amazon_search_scraper.py <query>", file=sys.stderr)
        raise SystemExit(1)
    query = sys.argv[1]
    data = scrape(query)
