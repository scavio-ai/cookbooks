"""
eBay seller profile scraper -- pure Scavio API scraper.

Scrapes data straight from the Scavio API endpoint POST /api/v1/ebay/seller.
...
Usage:
  python ebay_seller_scraper.py "red-rock-uk"
"""
...
API_URL = "https://api.scavio.dev/api/v1/ebay/seller"


def scrape(seller: str) -> dict:
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python ebay_seller_scraper.py 'red-rock-uk'", file=sys.stderr)
        raise SystemExit(1)
    seller = sys.argv[1]
    data = scrape(seller)
