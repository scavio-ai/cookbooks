"""
Threads user search scraper -- pure Scavio API scraper.

Scrapes data straight from the Scavio API endpoint POST /api/v1/threads/search/users.
...
Usage:
  python threads_search_users_scraper.py "technology"
"""
...
API_URL = "https://api.scavio.dev/api/v1/threads/search/users"
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python threads_search_users_scraper.py 'technology'", file=sys.stderr)
        raise SystemExit(1)
    query = sys.argv[1]
    data = scrape(query)
