"""
X (Twitter) search scraper -- pure Scavio API scraper.

Scrapes data straight from the Scavio API endpoint POST /api/v1/x/search.
...
Usage:
  python x_search_scraper.py "artificial intelligence"
"""
...
API_URL = "https://api.scavio.dev/api/v1/x/search"
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python x_search_scraper.py 'artificial intelligence'", file=sys.stderr)
        raise SystemExit(1)
    search = sys.argv[1]
    data = scrape(search)
