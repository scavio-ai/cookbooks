"""
Yelp business search scraper -- pure Scavio API scraper.

Scrapes data straight from the Scavio API endpoint POST /api/v1/yelp/search.
...
Usage:
  python yelp_search_scraper.py "coffee" "Austin, TX"
"""
...
API_URL = "https://api.scavio.dev/api/v1/yelp/search"


def scrape(term: str, location: str) -> dict:
    ...
    resp = requests.post(
        ...
        json={"term": term, "location": location},
        timeout=60,
    )
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print("Usage: python yelp_search_scraper.py <term> <location>", file=sys.stderr)
        raise SystemExit(1)
