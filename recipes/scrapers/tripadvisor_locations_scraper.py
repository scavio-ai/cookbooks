"""
TripAdvisor location lookup scraper -- pure Scavio API scraper.

Scrapes data straight from the Scavio API endpoint POST /api/v1/tripadvisor/locations.
...
Usage:
  python tripadvisor_locations_scraper.py "Austin"
"""
...
API_URL = "https://api.scavio.dev/api/v1/tripadvisor/locations"
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python tripadvisor_locations_scraper.py 'Austin'", file=sys.stderr)
        raise SystemExit(1)
    query = sys.argv[1]
    data = scrape(query)
