"""
Instagram profile scraper -- pure Scavio API scraper.

Scrapes an Instagram profile by username (followers, posts, bio, category),
straight from the Scavio API endpoint POST /api/v1/instagram/profile. No SDK, no
...
Usage:
  python instagram_profile_scraper.py "nike"
"""
...
API_URL = "https://api.scavio.dev/api/v1/instagram/profile"
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python instagram_profile_scraper.py <username>", file=sys.stderr)
        raise SystemExit(1)
    username = sys.argv[1]
