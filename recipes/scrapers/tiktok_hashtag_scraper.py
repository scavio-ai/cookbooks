"""
TikTok hashtag scraper -- pure Scavio API scraper.

Scrapes TikTok hashtag info and id by name,
straight from the Scavio API endpoint POST /api/v1/tiktok/hashtag. No SDK, no
...
Usage:
  python tiktok_hashtag_scraper.py "coffee"
"""
...
API_URL = "https://api.scavio.dev/api/v1/tiktok/hashtag"


def scrape(hashtag_name: str) -> dict:
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python tiktok_hashtag_scraper.py <hashtag_name>", file=sys.stderr)
        raise SystemExit(1)
    hashtag_name = sys.argv[1]
    data = scrape(hashtag_name)
