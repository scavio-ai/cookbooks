"""
YouTube search scraper -- pure Scavio API scraper.

Scrapes YouTube video, channel, and playlist search results,
straight from the Scavio API endpoint POST /api/v1/youtube/search. No SDK, no
...
Usage:
  python youtube_search_scraper.py "python tutorial"
"""
...
API_URL = "https://api.scavio.dev/api/v1/youtube/search"
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python youtube_search_scraper.py <query>", file=sys.stderr)
        raise SystemExit(1)
    query = sys.argv[1]
    data = scrape(query)
