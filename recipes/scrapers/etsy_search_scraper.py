"""
Etsy product search scraper -- pure Scavio API scraper.

Scrapes data straight from the Scavio API endpoint POST /api/v1/etsy/search.
...
Usage:
  python etsy_search_scraper.py "ceramic mug"
"""
...
API_URL = "https://api.scavio.dev/api/v1/etsy/search"
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python etsy_search_scraper.py 'ceramic mug'", file=sys.stderr)
        raise SystemExit(1)
    query = sys.argv[1]
    data = scrape(query)
