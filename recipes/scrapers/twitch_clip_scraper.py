"""
Twitch clip scraper -- pure Scavio API scraper.

Scrapes data straight from the Scavio API endpoint POST /api/v1/twitch/clip.
...
Usage:
  python twitch_clip_scraper.py "AwkwardHelplessSalamanderSwiftRage"
"""
...
API_URL = "https://api.scavio.dev/api/v1/twitch/clip"


def scrape(clip: str) -> dict:
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python twitch_clip_scraper.py 'AwkwardHelplessSalamanderSwiftRage'", file=sys.stderr)
        raise SystemExit(1)
    clip = sys.argv[1]
    data = scrape(clip)
