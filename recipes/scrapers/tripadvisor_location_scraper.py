"""
TripAdvisor location detail scraper -- pure Scavio API scraper.

Scrapes data straight from the Scavio API endpoint POST /api/v1/tripadvisor/location.
...
Usage:
  python tripadvisor_location_scraper.py "30196"
"""
...
API_URL = "https://api.scavio.dev/api/v1/tripadvisor/location"


def scrape(location_id: str) -> dict:
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python tripadvisor_location_scraper.py '30196'", file=sys.stderr)
        raise SystemExit(1)
    location_id = sys.argv[1]
    data = scrape(location_id)
