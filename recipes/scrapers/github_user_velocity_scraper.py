"""
GitHub user activity velocity scraper -- pure Scavio API scraper.

Scrapes data straight from the Scavio API endpoint POST /api/v1/github/user/profile-velocity.
...
Usage:
  python github_user_velocity_scraper.py "torvalds"
"""
...
API_URL = "https://api.scavio.dev/api/v1/github/user/profile-velocity"


def scrape(handle: str) -> dict:
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python github_user_velocity_scraper.py 'torvalds'", file=sys.stderr)
        raise SystemExit(1)
    handle = sys.argv[1]
    data = scrape(handle)
