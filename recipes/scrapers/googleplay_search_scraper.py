"""
Google Play app search scraper -- pure Scavio API scraper.

Scrapes data straight from the Scavio API endpoint POST /api/v1/googleplay/search.
...
Usage:
  python googleplay_search_scraper.py "todo list"
"""
...
API_URL = "https://api.scavio.dev/api/v1/googleplay/search"
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python googleplay_search_scraper.py 'todo list'", file=sys.stderr)
        raise SystemExit(1)
    query = sys.argv[1]
    data = scrape(query)
