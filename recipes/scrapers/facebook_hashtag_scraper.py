"""
Facebook hashtag scraper -- pure Scavio API scraper.

Scrapes data straight from the Scavio API endpoint POST /api/v1/facebook/hashtag.
...
Usage:
  python facebook_hashtag_scraper.py "photography"
"""
...
API_URL = "https://api.scavio.dev/api/v1/facebook/hashtag"


def scrape(tag: str) -> dict:
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python facebook_hashtag_scraper.py 'photography'", file=sys.stderr)
        raise SystemExit(1)
    tag = sys.argv[1]
    data = scrape(tag)
