"""
GitHub repo top issues scraper -- pure Scavio API scraper.

Scrapes data straight from the Scavio API endpoint POST /api/v1/github/repo/top-issues.
...
Usage:
  python github_repo_top_issues_scraper.py "facebook/react"
"""
...
API_URL = "https://api.scavio.dev/api/v1/github/repo/top-issues"
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python github_repo_top_issues_scraper.py 'facebook/react'", file=sys.stderr)
        raise SystemExit(1)
    url = sys.argv[1]
    data = scrape(url)
