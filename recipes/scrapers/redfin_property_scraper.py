"""
Redfin property detail scraper -- pure Scavio API scraper.

Scrapes data straight from the Scavio API endpoint POST /api/v1/redfin/property.
...
Usage:
  python redfin_property_scraper.py "31295086"
"""
...
API_URL = "https://api.scavio.dev/api/v1/redfin/property"


def scrape(property_id: str) -> dict:
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python redfin_property_scraper.py '31295086'", file=sys.stderr)
        raise SystemExit(1)
    property_id = sys.argv[1]
    data = scrape(property_id)
