"""
Redfin property search scraper -- pure Scavio API scraper.

Scrapes data straight from the Scavio API endpoint POST /api/v1/redfin/search.
...
Usage:
  python redfin_search_scraper.py "Austin, TX"
"""
...
API_URL = "https://api.scavio.dev/api/v1/redfin/search"
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python redfin_search_scraper.py 'Austin, TX'", file=sys.stderr)
        raise SystemExit(1)
    query = sys.argv[1]
    data = scrape(query)
