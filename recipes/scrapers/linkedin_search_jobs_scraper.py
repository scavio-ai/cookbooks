"""
LinkedIn job search scraper -- pure Scavio API scraper.

Scrapes data straight from the Scavio API endpoint POST /api/v1/linkedin/search/jobs.
...
Usage:
  python linkedin_search_jobs_scraper.py "software engineer"
"""
...
API_URL = "https://api.scavio.dev/api/v1/linkedin/search/jobs"
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python linkedin_search_jobs_scraper.py 'software engineer'", file=sys.stderr)
        raise SystemExit(1)
    search = sys.argv[1]
    data = scrape(search)
