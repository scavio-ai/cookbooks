"""
Target product search scraper -- pure Scavio API scraper.

Scrapes data straight from the Scavio API endpoint POST /api/v1/target/search.
...
Usage:
  python target_search_scraper.py "airpods"
"""
...
API_URL = "https://api.scavio.dev/api/v1/target/search"
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python target_search_scraper.py 'airpods'", file=sys.stderr)
        raise SystemExit(1)
    keyword = sys.argv[1]
    data = scrape(keyword)
