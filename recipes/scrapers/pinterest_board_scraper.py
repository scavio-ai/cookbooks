"""
Pinterest board pins scraper -- pure Scavio API scraper.

Scrapes data straight from the Scavio API endpoint POST /api/v1/pinterest/board.
...
Usage:
  python pinterest_board_scraper.py "pinterest/spice-up-your-dinner-plans"
"""
...
API_URL = "https://api.scavio.dev/api/v1/pinterest/board"


def scrape(board: str) -> dict:
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python pinterest_board_scraper.py 'pinterest/spice-up-your-dinner-plans'", file=sys.stderr)
        raise SystemExit(1)
    board = sys.argv[1]
    data = scrape(board)
