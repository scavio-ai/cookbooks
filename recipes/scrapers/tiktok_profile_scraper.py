"""
TikTok profile scraper -- pure Scavio API scraper.

Scrapes a TikTok user profile by username (followers, hearts, bio, sec_uid),
straight from the Scavio API endpoint POST /api/v1/tiktok/profile. No SDK, no
...
Usage:
  python tiktok_profile_scraper.py "tiktok"
"""
...
API_URL = "https://api.scavio.dev/api/v1/tiktok/profile"
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python tiktok_profile_scraper.py <username>", file=sys.stderr)
        raise SystemExit(1)
    username = sys.argv[1]
