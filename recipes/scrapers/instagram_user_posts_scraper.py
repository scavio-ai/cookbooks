"""
Instagram user posts scraper -- pure Scavio API scraper.

Scrapes an Instagram user's recent posts with likes and comments,
straight from the Scavio API endpoint POST /api/v1/instagram/user/posts. No SDK, no
...
Usage:
  python instagram_user_posts_scraper.py "nike"
"""
...
API_URL = "https://api.scavio.dev/api/v1/instagram/user/posts"
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python instagram_user_posts_scraper.py <username>", file=sys.stderr)
        raise SystemExit(1)
    username = sys.argv[1]
