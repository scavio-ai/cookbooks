"""
LinkedIn company posts scraper -- pure Scavio API scraper.

Scrapes data straight from the Scavio API endpoint POST /api/v1/linkedin/company/posts.
...
Usage:
  python linkedin_company_posts_scraper.py "microsoft"
"""
...
API_URL = "https://api.scavio.dev/api/v1/linkedin/company/posts"


def scrape(company: str) -> dict:
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python linkedin_company_posts_scraper.py 'microsoft'", file=sys.stderr)
        raise SystemExit(1)
    company = sys.argv[1]
    data = scrape(company)
