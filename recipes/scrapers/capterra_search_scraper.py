"""
Capterra software search scraper -- pure Scavio API scraper.

Scrapes data straight from the Scavio API endpoint POST /api/v1/capterra/search.
...
Usage:
  python capterra_search_scraper.py "project management"
"""
...
API_URL = "https://api.scavio.dev/api/v1/capterra/search"
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python capterra_search_scraper.py 'project management'", file=sys.stderr)
        raise SystemExit(1)
    query = sys.argv[1]
    data = scrape(query)
