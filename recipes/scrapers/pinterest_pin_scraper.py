"""
Pinterest pin detail scraper -- pure Scavio API scraper.

Scrapes data straight from the Scavio API endpoint POST /api/v1/pinterest/pin.
...
Usage:
  python pinterest_pin_scraper.py "104779128829676115"
"""
...
API_URL = "https://api.scavio.dev/api/v1/pinterest/pin"


def scrape(pin: str) -> dict:
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python pinterest_pin_scraper.py '104779128829676115'", file=sys.stderr)
        raise SystemExit(1)
    pin = sys.argv[1]
    data = scrape(pin)
