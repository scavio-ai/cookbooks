"""
Walmart search scraper -- pure Scavio API scraper.

Scrapes Walmart product search results with price, rating, and fulfillment,
straight from the Scavio API endpoint POST /api/v1/walmart/search. No SDK, no
...
Usage:
  python walmart_search_scraper.py "coffee maker"
"""
...
API_URL = "https://api.scavio.dev/api/v1/walmart/search"
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python walmart_search_scraper.py <query>", file=sys.stderr)
        raise SystemExit(1)
    query = sys.argv[1]
    data = scrape(query)
