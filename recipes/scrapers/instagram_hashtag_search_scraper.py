"""
Instagram hashtag search scraper -- pure Scavio API scraper.

Scrapes Instagram hashtags matching a keyword with media counts,
straight from the Scavio API endpoint POST /api/v1/instagram/search/hashtags. No SDK, no
...
Usage:
  python instagram_hashtag_search_scraper.py "coffee"
"""
...
API_URL = "https://api.scavio.dev/api/v1/instagram/search/hashtags"
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python instagram_hashtag_search_scraper.py <keyword>", file=sys.stderr)
        raise SystemExit(1)
    keyword = sys.argv[1]
    data = scrape(keyword)
