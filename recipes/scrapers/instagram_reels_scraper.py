"""
Instagram reels scraper -- pure Scavio API scraper.

Scrapes an Instagram user's reels with play and like counts,
straight from the Scavio API endpoint POST /api/v1/instagram/user/reels. No SDK, no
...
Usage:
  python instagram_reels_scraper.py "nike"
"""
...
API_URL = "https://api.scavio.dev/api/v1/instagram/user/reels"
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python instagram_reels_scraper.py <username>", file=sys.stderr)
        raise SystemExit(1)
    username = sys.argv[1]
