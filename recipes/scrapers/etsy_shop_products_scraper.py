"""
Etsy shop listings scraper -- pure Scavio API scraper.

Scrapes data straight from the Scavio API endpoint POST /api/v1/etsy/shop/products.
...
Usage:
  python etsy_shop_products_scraper.py "StonehousePotteryOH"
"""
...
API_URL = "https://api.scavio.dev/api/v1/etsy/shop/products"


def scrape(shop: str) -> dict:
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python etsy_shop_products_scraper.py 'StonehousePotteryOH'", file=sys.stderr)
        raise SystemExit(1)
    shop = sys.argv[1]
    data = scrape(shop)
