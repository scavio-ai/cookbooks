"""
TikTok Shop search suggestions scraper -- pure Scavio API scraper.

Scrapes data straight from the Scavio API endpoint POST /api/v1/tiktok-shop/search/suggestions.
...
Usage:
  python tiktok_shop_suggestions_scraper.py "phone"
"""
...
API_URL = "https://api.scavio.dev/api/v1/tiktok-shop/search/suggestions"


def scrape(search: str) -> dict:
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python tiktok_shop_suggestions_scraper.py 'phone'", file=sys.stderr)
        raise SystemExit(1)
    search = sys.argv[1]
    data = scrape(search)
