"""
Home Depot product reviews scraper -- pure Scavio API scraper.

Scrapes data straight from the Scavio API endpoint POST /api/v1/homedepot/reviews.
...
Usage:
  python homedepot_reviews_scraper.py "326828759"
"""
...
API_URL = "https://api.scavio.dev/api/v1/homedepot/reviews"


def scrape(item_id: str) -> dict:
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python homedepot_reviews_scraper.py '326828759'", file=sys.stderr)
        raise SystemExit(1)
    item_id = sys.argv[1]
    data = scrape(item_id)
