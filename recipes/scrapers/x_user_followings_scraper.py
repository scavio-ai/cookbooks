"""
X (Twitter) user followings scraper -- pure Scavio API scraper.

Scrapes data straight from the Scavio API endpoint POST /api/v1/x/user/followings.
...
Usage:
  python x_user_followings_scraper.py "elonmusk"
"""
...
API_URL = "https://api.scavio.dev/api/v1/x/user/followings"


def scrape(screen_name: str) -> dict:
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python x_user_followings_scraper.py 'elonmusk'", file=sys.stderr)
        raise SystemExit(1)
    screen_name = sys.argv[1]
    data = scrape(screen_name)
