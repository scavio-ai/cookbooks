"""
X (Twitter) tweet retweeters scraper -- pure Scavio API scraper.

Scrapes data straight from the Scavio API endpoint POST /api/v1/x/tweet/retweeters.
...
Usage:
  python x_tweet_retweeters_scraper.py "1808168603721650364"
"""
...
API_URL = "https://api.scavio.dev/api/v1/x/tweet/retweeters"


def scrape(tweet_id: str) -> dict:
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python x_tweet_retweeters_scraper.py '1808168603721650364'", file=sys.stderr)
        raise SystemExit(1)
    tweet_id = sys.argv[1]
    data = scrape(tweet_id)
