"""
Meta Ad Library advertiser scraper -- pure Scavio API scraper.

Scrapes data straight from the Scavio API endpoint POST /api/v1/meta-ads/advertiser.
...
Usage:
  python metaads_advertiser_scraper.py "15087023444"
"""
...
API_URL = "https://api.scavio.dev/api/v1/meta-ads/advertiser"


def scrape(page_id: str) -> dict:
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python metaads_advertiser_scraper.py '15087023444'", file=sys.stderr)
        raise SystemExit(1)
    page_id = sys.argv[1]
    data = scrape(page_id)
