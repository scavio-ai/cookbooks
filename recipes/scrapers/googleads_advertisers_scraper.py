"""
Google Ads advertiser lookup scraper -- pure Scavio API scraper.

Scrapes data straight from the Scavio API endpoint POST /api/v1/googleads/advertisers.
...
Usage:
  python googleads_advertisers_scraper.py "Peloton"
"""
...
API_URL = "https://api.scavio.dev/api/v1/googleads/advertisers"
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python googleads_advertisers_scraper.py 'Peloton'", file=sys.stderr)
        raise SystemExit(1)
    query = sys.argv[1]
    data = scrape(query)
