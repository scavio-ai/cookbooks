"""
Google Ads Transparency search scraper -- pure Scavio API scraper.

Scrapes data straight from the Scavio API endpoint POST /api/v1/googleads/search.
...
Usage:
  python googleads_search_scraper.py "AR01234567890123456"
"""
...
API_URL = "https://api.scavio.dev/api/v1/googleads/search"


def scrape(advertiser_id: str) -> dict:
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python googleads_search_scraper.py 'AR01234567890123456'", file=sys.stderr)
        raise SystemExit(1)
    advertiser_id = sys.argv[1]
    data = scrape(advertiser_id)
