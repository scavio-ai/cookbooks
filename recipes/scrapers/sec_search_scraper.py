"""
SEC EDGAR full-text search scraper -- pure Scavio API scraper.

Scrapes data straight from the Scavio API endpoint POST /api/v1/sec/search.
...
Usage:
  python sec_search_scraper.py "climate risk"
"""
...
API_URL = "https://api.scavio.dev/api/v1/sec/search"
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python sec_search_scraper.py 'climate risk'", file=sys.stderr)
        raise SystemExit(1)
    query = sys.argv[1]
    data = scrape(query)
