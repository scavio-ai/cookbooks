"""
Indeed job detail scraper -- pure Scavio API scraper.

Scrapes data straight from the Scavio API endpoint POST /api/v1/indeed/job.
...
Usage:
  python indeed_job_scraper.py "db268b11ac6ba55b"
"""
...
API_URL = "https://api.scavio.dev/api/v1/indeed/job"


def scrape(job_id: str) -> dict:
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python indeed_job_scraper.py 'db268b11ac6ba55b'", file=sys.stderr)
        raise SystemExit(1)
    job_id = sys.argv[1]
    data = scrape(job_id)
