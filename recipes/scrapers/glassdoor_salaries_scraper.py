"""
Glassdoor salary data scraper -- pure Scavio API scraper.

Scrapes data straight from the Scavio API endpoint POST /api/v1/glassdoor/salaries.
...
Usage:
  python glassdoor_salaries_scraper.py "1699"
"""
...
API_URL = "https://api.scavio.dev/api/v1/glassdoor/salaries"


def scrape(employer_id: str) -> dict:
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python glassdoor_salaries_scraper.py '1699'", file=sys.stderr)
        raise SystemExit(1)
    employer_id = sys.argv[1]
    data = scrape(employer_id)
