"""
TikTok user posts scraper -- pure Scavio API scraper.

Scrapes a TikTok user's posted videos by sec_user_id (from a profile scrape),
straight from the Scavio API endpoint POST /api/v1/tiktok/user/posts. No SDK, no
...
Usage:
  python tiktok_user_posts_scraper.py "MS4wL..."
"""
...
API_URL = "https://api.scavio.dev/api/v1/tiktok/user/posts"
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python tiktok_user_posts_scraper.py <sec_user_id>", file=sys.stderr)
        raise SystemExit(1)
