"""
Companies House search scraper -- pure Scavio API scraper.

Scrapes data straight from the Scavio API endpoint POST /api/v1/companieshouse/search.
...
Usage:
  python companieshouse_search_scraper.py "Deliveroo"
"""
...
API_URL = "https://api.scavio.dev/api/v1/companieshouse/search"
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python companieshouse_search_scraper.py 'Deliveroo'", file=sys.stderr)
        raise SystemExit(1)
    query = sys.argv[1]
    data = scrape(query)
