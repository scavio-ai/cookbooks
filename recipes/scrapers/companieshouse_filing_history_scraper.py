"""
Companies House filing history scraper -- pure Scavio API scraper.

Scrapes data straight from the Scavio API endpoint POST /api/v1/companieshouse/filing-history.
...
Usage:
  python companieshouse_filing_history_scraper.py "09397466"
"""
...
API_URL = "https://api.scavio.dev/api/v1/companieshouse/filing-history"


def scrape(company_number: str) -> dict:
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python companieshouse_filing_history_scraper.py '09397466'", file=sys.stderr)
        raise SystemExit(1)
    company_number = sys.argv[1]
    data = scrape(company_number)
