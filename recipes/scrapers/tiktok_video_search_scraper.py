"""
TikTok video search scraper -- pure Scavio API scraper.

Scrapes TikTok videos matching a keyword,
straight from the Scavio API endpoint POST /api/v1/tiktok/search/videos. No SDK, no
...
Usage:
  python tiktok_video_search_scraper.py "coffee"
"""
...
API_URL = "https://api.scavio.dev/api/v1/tiktok/search/videos"
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python tiktok_video_search_scraper.py <keyword>", file=sys.stderr)
        raise SystemExit(1)
    keyword = sys.argv[1]
    data = scrape(keyword)
